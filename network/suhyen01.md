...
#### connection 수립 과정
> 편의를 위해 연결을 먼저 요청하는 Host를 client, 연결을 요청 받는 Host를 server로 설명

  1) client : SYNbit=1, seq#=x
  2) server : ACKbit=1, ACK#=x+1 / SYNbit=1, seq#=y
...
<details>
<summary>1단계: [Client -> Server] Seq#=100, Ack#=0 + Flags(SYN=1, ACK=0) + Options(MSS=1460)</summary>

- Seq#=100: 클라이언트 시작 기준점이 100번임을 알림
- SYN=1: 최초 TCP 연결 수립 요청
- MSS=1460: 클라이언트가 받을 수 있는 최대 Payload 크기가 1460byte임을 알림
![](./images/suhyen/tcp3-1.png)
...
<details>
<summary>2단계: [Client <- Server] Seq#=2000, Ack#=101 + Flags(SYN=1, ACK=1) + Options(MSS=1000)</summary>
...
![](./images/suhyen/tcp3-2.png)

</details>

<details>
<summary>3단계: [Client -> Server] Seq#=101, Ack#=2001 + Flags(SYN=0, ACK=1) + Options()</summary>
...
<details>
<summary>Host(Client, Server)끼리 주고받는 패킷/세그먼트의 크기 (IPv4 기준)</summary>
...
![](./images/suhyen/2way-handshake.png)
...
![](./images/suhyen/tcp-simultaneous.png)
...
![](./images/suhyen/syn-flooding.png)
